...
int numVal(int num, int min, int max){
  if(num > max || num < min){
    cout<<"ERROR, el numero introducido no se encuentra dentro del rango establecido ("<<min<<" - "<<max<<"). Intente nuevamente: ";
    cin>> num ;
...
string nombreDif(Jugador *inicio, string nom){
    ...
    while(mover != NULL){
        if(nom == mover->nombre){
                cout<<"El nombre ingresado ya fue ocupado, por favor intente nuevamente: ";
                cin >> nom;
...
Jugador *crearJugador(Jugador *inicio, int numjug){
    ...
    switch (contrand){
        case 1:{
          ...
          nuevo->contnum = 1;
          nuevo->regiones = crearRegiones("America del Norte", "Mesoamerica", "America Central", "America del Sur", "Las Antillas");
          break;}
        
        case 2:{
          nuevo->contnom = "EUROPA";
          nuevo->contnum = 2;
          nuevo->regiones = crearRegiones("Escandinavia", "Europa Occidental", "Europa Oriental", "Cuenca del Mediterraneo", "Rusia");
          break; }
        
        case 3: {
          nuevo->contnom = "ASIA Y OCEANIA";
          nuevo->contnum = 3;
          nuevo->regiones = crearRegiones("Medio Oriente", "Asia Meridional", "Asia Oriental", "Sureste de Asia",  "Oceania");
          break; } 

        case 4: {
          nuevo->contnom = "AFRICA";
          nuevo->contnum = 4;
          nuevo->regiones = crearRegiones("Africa Septentrional", "Africa Occidental", "Africa Central", "Africa Oriental", "Africa Austral");
          break; } 
    }
    return nuevo;
}
...
void mostrarContAsig(Jugador* actual){
  while(actual != NULL){
    cout<<" A el jugador '"<<actual->nombre<<"' le corresponde proteger al continente: "<<actual->contnom<<endl;
    actual = actual->prox;
  }
} 

void mostrarRe(Region* inicio, Region* actualRe){
  cout<<"Utiliza las teclas '∧' y '∨' para seleccionar la region que deseas visitar."<<endl;
...
Region* selectRe(Jugador* actual){
  ...
  do{    
    ...
    if(input == 72 && actualRe->prevre != NULL){
      ...
      cout<<"Se ha seleccionado la region: " + actualRe->renom<<endl<<endl;
    }
  } while(input != 13);
  return actualRe;
}
